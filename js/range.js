...
function runRange(selector) {
  ...
  let prevV = 0;
  let isScrollRight = null; // to get val use 
  ...
  $(selector + ' input[type="range"]').rangeslider({
    ...
    onSlide: (p, v) => {
      ...
      for (let i = 0; i < 10; i++) {
        i < v ? parts[i].classList.remove('inactive') : parts[i].classList.add('inactive');
      }
    },
    onSlideEnd: (p, v) => {
      prevV = v;
...
runRange('#range1'); // const rangeThumb = document.querySelector('.range-thumb');
